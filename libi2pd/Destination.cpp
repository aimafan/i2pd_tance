...
namespace client
{
	...
	{
		bool found = false;		// 标记是否找到了合适的floodfill结点
		if (request->excluded.size () < MAX_NUM_FLOODFILLS_PER_REQUEST)		// 排除的必须小于7个
		{
			auto floodfill = i2p::data::netdb.GetClosestFloodfill (key, request->excluded);		// 获得距离key最近的floodfill
			if (floodfill)		// 如果找到，向他发送请求
			{
	...
	void LeaseSetDestination::HandlePublishVerificationTimer (const boost::system::error_code& ecode)
	{
		if (ecode != boost::asio::error::operation_aborted)
		{
			auto ls = GetLeaseSetMt ();		// 获得本地的LeaseSet
			if (!ls)
			{
	...
	// 请求LeaseSet
	// dest 请求leaseset的IdentHash
	// requestComplete 
	// requestedBlindedKey 用于加密请求
	...
	{
		std::set<i2p::data::IdentHash> excluded;		// 被排除的结点
		auto floodfill = i2p::data::netdb.GetClosestFloodfill (dest, excluded);		// 获得最近的floodfill结点
		if (floodfill)
		{
			auto request = std::make_shared<LeaseSetRequest> (m_Service);		// 创建一个LeaseSetRequest对象
			request->requestedBlindedKey = requestedBlindedKey; // for encrypted LeaseSet2
			if (requestComplete)
				request->requestComplete.push_back (requestComplete);
			auto ts = i2p::util::GetSecondsSinceEpoch ();
			// 这里也有一个m_LeaseSetRequests
