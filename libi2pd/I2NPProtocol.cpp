...
namespace i2p
{
	...
	{
		int cnt = excludedPeers ? excludedPeers->size () : 0;	// 大小为0
		auto m = cnt > 7 ? NewI2NPMessage () : NewI2NPShortMessage ();		// 创建一个新的I2NP消息
		uint8_t * buf = m->GetPayload ();	// I2NP中的负载
		// 把目的和自己写到I2NP当中
		memcpy (buf, key, 32); // key
		buf += 32;
		memcpy (buf, from, 32); // from
		buf += 32;
		uint8_t flag = exploratory ? DATABASE_LOOKUP_TYPE_EXPLORATORY_LOOKUP : DATABASE_LOOKUP_TYPE_ROUTERINFO_LOOKUP;	// 是探索
		if (replyTunnelID)
		{
