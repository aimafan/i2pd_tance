...
namespace data
{
	...
	// 3 构造函数
	// m_StoreLeases：是否需要存储lease
	// m_ExpirationTime：过期时间
		...
		m_IsValid (true), m_StoreLeases (storeLeases), m_ExpirationTime (0), m_EncryptionKey (nullptr)
	{
		...
		memcpy (m_Buffer, buf, len);	// 将buf数据传入到m_Buffer中
		m_BufferLen = len;
		ReadFromBuffer ();
	}
	...
	{
		// m_Buffer组成：
		// 1. m_Identity身份信息：长度size
		// 2. m_EncryptionKey加密密钥：长度256
		// 从缓冲区中创建一个身份信息
		...
		if (!num || num > MAX_NUM_LEASES)		//num等于0或者是num大于16
		{
		...
		m_ExpirationTime += LEASE_ENDDATE_THRESHOLD;	// 租约结束时间后面加51秒
		UpdateLeasesEnd ();
	...
	{
		// standard LS2 header
		std::shared_ptr<const IdentityEx> identity;		// 定义一个身份
