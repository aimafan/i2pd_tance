...
namespace i2p
{
	namespace transport
	{
		NTCP2Establisher::NTCP2Establisher() : m_SessionConfirmedBuffer(nullptr)
		{
		}
		...
		NTCP2Session::NTCP2Session(NTCP2Server &server, std::shared_ptr<const i2p::data::RouterInfo> in_RemoteRouter,
								   std::shared_ptr<const i2p::data::RouterInfo::Address> addr) : TransportSession(in_RemoteRouter, NTCP2_ESTABLISH_TIMEOUT),
...
#if OPENSSL_SIPHASH
			if (m_SendMDCtx)
				EVP_MD_CTX_destroy(m_SendMDCtx);
			if (m_ReceiveMDCtx)
				EVP_MD_CTX_destroy(m_ReceiveMDCtx);
#endif
		}
			...
			else
			{
				...
				if (m_Establisher->ProcessSessionConfirmedMessagePart1(nonce))
				{
					...
					if (m_Establisher->ProcessSessionConfirmedMessagePart2(nonce, buf.data()))
					{
						...
						auto addr = m_RemoteEndpoint.address().is_v4() ? ri.GetNTCP2V4Address() : (i2p::util::net::IsYggdrasilAddress(m_RemoteEndpoint.address()) ? ri.GetYggdrasilAddress() : ri.GetNTCP2V6Address());
						...
						if (addr->IsPublishedNTCP2() && m_RemoteEndpoint.address() != addr->host &&
							(!m_RemoteEndpoint.address().is_v6() || (i2p::util::net::IsYggdrasilAddress(m_RemoteEndpoint.address()) ? memcmp(m_RemoteEndpoint.address().to_v6().to_bytes().data() + 1, addr->host.to_v6().to_bytes().data() + 1, 7) : // from the same yggdrasil subnet
		...
		size_t NTCP2Session::CreatePaddingBlock(size_t msgLen, uint8_t *buf, size_t len)
		{
			...
			len -= 3;
			if (msgLen < 256)
				msgLen = 256; // for short message padding should not be always zero
		...
		NTCP2Server::NTCP2Server() : RunnableServiceWithWork("NTCP2"), m_TerminationTimer(GetService()),
				...
				for (const auto &address : *addresses)
				{
					...
					if (address->IsPublishedNTCP2() && address->port)
					{
						if (address->IsV4())
						{
							try
							{
								auto ep = m_Address4 ? boost::asio::ip::tcp::endpoint(m_Address4->address(), address->port) : boost::asio::ip::tcp::endpoint(boost::asio::ip::tcp::v4(), address->port);
